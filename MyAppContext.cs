...
namespace EpsonConnector
{
    public class MyAppContext : ApplicationContext
    {
        ...
        public MyAppContext()
        {
            ...
            if (string.IsNullOrEmpty(Settings.Default.PrinterName) || string.IsNullOrEmpty(Settings.Default.EditorLocation) || string.IsNullOrEmpty(Settings.Default.SunucuIp))
        ...
        private async void ControlAndPrint(object sender, System.Timers.ElapsedEventArgs e)
        {
            if (/*printer.IsConnected &&*/ !IsBusy)
            {
                ...
                if (assetsToPrint.Count > 0)
                {
                    ...
                    printer.PrintAll(assetsToPrint);
